...
function calcDuration(blocks, deletedBlockIndices = new Set()) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const endSec = nextB ? tsToSeconds(nextB.timestamp) : (startSec + 10); // 마지막 블록은 10초 추정
                      ...
                      {ci ? (
                        <>
                          <div style={{display:"flex",alignItems:"baseline",gap:8}}>
                            <div style={{fontSize:24,fontWeight:800,color:"#22C55E"}}>{secondsToDisplay(ci.pointSec)}</div>
                            {ci.method === "TS+밀도" && <span style={{fontSize:10,padding:"2px 6px",borderRadius:4,background:"rgba(34,197,94,0.08)",color:C.txD}}>TS+밀도</span>}
                          </div>
                        ...
                        <>
                          <div style={{fontSize:24,fontWeight:800,color:"#22C55E"}}>{secondsToDisplay(duration.keptSeconds)}</div>
